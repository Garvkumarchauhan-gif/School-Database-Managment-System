...
class School_Portal():
   ...
   def run_query(self,query,parameters=()):
       with _sqlite3.connect(self.db_name)as conn:
          cursor=conn.cursor()
   ...
   def viewing_records(self):
       ...
       for data in db_table:
           self.tree.insert('',1000,text=[0],values=data[1:])

   #------------------Add New REcord-------------------
   def validation(self):
     return len(self.firstname.get()) !=0 and len(self.lastname.get()) !=0 and  len(self.firstname.get()) !=0 and len(self.username.get()) !=0 and \
   ...
   #--------------Clear Fields----------------------
       ...
       self.message['text']='Field not completed! Fill all fields...'
   ...
   def edit_box(self):
       ...
       (new_fname.get(),fname,new_lname.get(),lname
        ,new_uname.get(),uname,new_email.get(),email,new_subject.get(),subject,new_age.get(),age))\
   ...
   def ex(self):
       exit= tkinter.messagebox.askquestion('Exit Applicaton','Want to close your app?')
       if exit == 'yes':
           root.destroy()
